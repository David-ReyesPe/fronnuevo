...
function Index() {
  ...
  const { nombre, titulos, description, images, estado } = Instructores;

  const { mutate, isLoading, isError } = useInstructoresMutation();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!titulos || !description || !nombre || !estado) {
      setError(true);
      return;
    }
        ...
        <div className="h-full p-5">
          <div className="block md:flex justify-center">
            <div className="w-full md:w-2/5 p-4 sm:p-6 lg:p-8 bg-white shadow-md border border-zinc-300 rounded-3xl ml-2">
              ...
              <select className="mb-4 block w-full rounded-lg border border-gray-300 px-4 py-3 focus:outline-none" id="estado" name="estado" value={estado} onChange={(e) => onInputChange(e)}>
